...
;; 
;; IMPORTANT: acl2-mcp supports two modes:
;;   1. One-shot mode (no session_id): Creates a new ACL2 process per call.
;;      This shows the full SBCL/ACL2 banner each time. Slow but stateless.
;;   2. Persistent session mode (with session_id): Uses a long-running ACL2
;;      process that maintains state. Much faster, no repeated banner.
;;
;; Use mcp-acl2-start-session to create a persistent session, then pass
;; the returned session-id to evaluate/admit/prove for fast execution.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; Start a persistent ACL2 session
;;
;; Parameters:
;;   conn       - MCP connection from mcp-connect
;;   name       - Optional human-readable name for the session (stringp or nil)
;;   state      - ACL2 state
;;
;; Returns: (mv error session-id state)
;;   error      - NIL on success, error string on failure
;;   session-id - The ACL2 session ID to pass to evaluate/admit/prove (stringp)
;;   state      - Updated state
(defun mcp-acl2-start-session (conn name state)
  (declare (xargs :guard (and (mcp-connection-p conn)
  ...
  (b* ((args (if name 
                 (list (cons "name" name))
               nil))
       ((mv err content state)
        (mcp-call-tool conn "start_session" args state))
       ((when err)
        (mv err "" state))
       ;; Extract session ID from response like "Session started successfully. ID: <uuid>"
       (id-pos (search "ID: " content))
       ((unless id-pos)
        (mv "Could not parse session ID from response" "" state))
       (id-start (+ id-pos 4))
       ;; UUID is 36 chars
       (session-id (if (<= (+ id-start 36) (length content))
                       (subseq content id-start (+ id-start 36))
                     "")))
...
;; Parameters:
;;   conn       - MCP connection from mcp-connect
;;   session-id - Session ID from mcp-acl2-start-session
;;   state      - ACL2 state
;;
;; Returns: (mv error result state)
(defun mcp-acl2-end-session (conn session-id state)
  (declare (xargs :guard (and (mcp-connection-p conn)
                              (stringp session-id))
                  :stobjs state))
...
;;   state      - ACL2 state
;;   session-id - Optional: persistent session ID for fast execution (stringp or nil)
;;
;; Returns: (mv error result state)
(defun mcp-acl2-evaluate (conn code state &optional session-id)
  (declare (xargs :guard (and (mcp-connection-p conn)
                              (stringp code)
                              (or (null session-id) (stringp session-id)))
                  :stobjs state))
  (let ((args (if session-id
                  (list (cons "code" code)
...
;;
;; Returns: (mv error result state)
(defun mcp-acl2-admit (conn code state &optional session-id)
  (declare (xargs :guard (and (mcp-connection-p conn)
                              (stringp code)
                              (or (null session-id) (stringp session-id)))
                  :stobjs state))
...
;;
;; Returns: (mv error result state)
(defun mcp-acl2-prove (conn code state &optional session-id)
  (declare (xargs :guard (and (mcp-connection-p conn)
                              (stringp code)
                              (or (null session-id) (stringp session-id)))
                  :stobjs state))
